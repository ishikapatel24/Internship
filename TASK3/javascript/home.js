...
function resizefunction(){
  ...
  else if(hamburgermenudisplay !== "none" && dashboardoptiondisplay === "flex")
  {
...
for (let i = 0; i < ln; i++) {
  ...
  <div class="filedate">
    <div class="file">
     ${ann.announcement[i].ig==1 ? `<img src="./image/fileAttach.png" alt="">&nbsp;`:``}${ann.announcement[i].files}
    </div>
  ...
  cards.innerHTML += `
    ...
    <div class="card1">
        ...
        <div class="carddetails">
            ...
            <div class="smalltext">${card.Carddetails[i].courseSubject} | Grade ${card.Carddetails[i].courseGrade}<span style="padding-left: 0.2rem; color: #1F7A54;">+${card.Carddetails[i].additionalCourseGrade}</span></div>
            <div class="smalltext"><span>${card.Carddetails[i].courseLength.units}</span> Units <span style="padding-left: 0.2rem;">${card.Carddetails[i].courseLength.lessons}</span> Lessons <span style="padding-left: 0.2rem;">${card.Carddetails[i].courseLength.topics}</span> Topics</div>
            <div class="bigtext" id="classdet">
                <div ${card.Carddetails[i].classoption===null
                  ?` style="color:#686868"`
                  :``
                  }>${card.Carddetails[i].classoption!==null ? card.Carddetails[i].classoption : `No Classes`}</div>
